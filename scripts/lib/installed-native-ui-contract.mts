...
export const nativePhoneWitnesses = {
  ...
  "settings-paths": ["diagnostics", "diagnostics-back", "usage", "usage-back", "watch", "license", "headers", "logs"],
  ...
  "fork-session-controls": ["fork", "remove-attachment", "fork-again", "reset", "new-thread", "return-child"],
  ...
  "notification-guidance": ["system-denied", "approval-event", "prompt", "refused", "not-now", "fresh-open"],
...
export type NativeWitnessID = keyof typeof nativePhoneWitnesses | keyof typeof nativeTabletWitnesses;

export function nativeUIPhases(kind: NativeUIKind): string[] {
  assert(kind === "phone" || kind === "tablet", "Unknown native UI inventory");
  const roster = kind === "phone" ? nativePhoneWitnesses : nativeTabletWitnesses;
  return ["onboarded", ...Object.entries(roster).flatMap(([id, phases]) =>
    [id + ":begin", ...phases.map((phase) => id + ":" + phase), id + ":complete"]), "complete"];
}
...
export function readNativeUIObservation(value: unknown): NativeUIObservation {
  ...
  for (const key of ["forwardingEntries", "forwardingCompletions"] as const) {
    assert(Number.isSafeInteger(value[key]) && Number(value[key]) >= 0 && Number(value[key]) < 256,
      "Native UI observation counter overflow or invalid value");
  }
  assert(typeof value.forwardingKind === "string" &&
    ["none", "session", "compose", "inspect"].includes(value.forwardingKind));
  assert(typeof value.forwardingOutcome === "string" &&
...
export function assertNativeUIForward(
  ...
  assert.equal(before.forwardingEntries, before.forwardingCompletions, "Previous native action is still owned");
...
export function assertNativeUITestResult(value: unknown, kind: NativeUIKind) {
  ...
  assert(identifier === expected || identifier === "OpenClawUITests/" + expected, "Wrong native UI selector result");
