...
- The direct evidence-verification `gh api` reads in `openclaw-npm-release.yml` stop on one HTTP 502. A bounded GET-only retry patch is prepared locally but deferred because the full replay consumed this run’s budget. It covers six trusted publisher metadata reads, discards partial failed responses, retries HTTP 5xx at most three times with 2s/4s backoff, and leaves 4xx and evidence rejection terminal. The patch still needs executable regression proof, changed checks, and fresh review; nested `release-ci-summary.mjs` reads remain a separate follow-up.
