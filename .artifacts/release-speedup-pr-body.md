...
- **Bootstrap authority.** The stable plugin-npm bootstrap approval records whether its soak waiver was explicit or sealed; a sealed waiver is honored at approval time, at the child's approval validation, and once more immediately before the token-backed `npm publish` (`assertStableSoakWaiverStillHeld`), each time only while `OPENCLAW_RELEASE_STABLE_SOAK_WAIVER` still holds the same text. Read-only preflight reports a sealed waiver as active only under the same condition.
