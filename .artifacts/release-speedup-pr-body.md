...
- **Bootstrap authority.** The stable plugin-npm bootstrap approval records whether its soak waiver was explicit or sealed; a sealed waiver is honored at approval time and again by the child's token-backed publish only while `OPENCLAW_RELEASE_STABLE_SOAK_WAIVER` still holds the same text. Read-only preflight reports a sealed waiver as active only under the same condition.
