...
This warning applies to pre-19 databases. If `foreign_key_check` names
`task_delivery_state` referencing `task_runs`, stop the Gateway, preserve a
verified WAL-aware backup, and run `openclaw doctor --fix` with the current build.
[State schema 19](/reference/database-schemas/state-schema-history#state-schema-19)
accepts only the recognized orphan relation as part of its fenced migration,
copies retained cron history and exact native outcomes, and drops the legacy
tables. Unrelated foreign-key violations, unrecognized source schemas, and
unknown dependencies still refuse migration.

Before that schema migration, the updater's existing-schema admission can
recover orphan delivery rows without advancing either schema marker or dropping
the Task tables. This separate preservation-first path saves a complete WAL-aware
database copy and a lossless row export in a private
`openclaw-task-delivery-recovery-*` directory beside the shared database. Its
report names that directory. It contains:
...
That existing-schema recovery removes only delivery rows whose task no longer
exists, inside its repair transaction, and requires clean integrity and
foreign-key checks before commit. It does not fabricate tasks or replay
deliveries. If preservation or a later repair step fails, row removal rolls back
and any recovery artifacts remain. A manifest proves preservation, not that the
repair committed. Current-schema validation does not recreate the retired tables
or run this legacy repair against state schema 19.
