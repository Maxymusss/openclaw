...
The [accepted Tasks and TaskFlow retirement](https://github.com/openclaw/openclaw/pull/156532#issuecomment-5798987497)
removes both the runtime APIs and their three shared-state tables in state schema 19. Cron history moves to its required `cron_run_history` table, preserving the
14 retained fields, raw detail JSON, identities, store partition, and existing
retention bounds. Before dropping the tables, migration copies only exact
released task-first native outcomes into `subagent_runs`, without dispatch,
revival, or inferred authority. Independent audit events and decision facts stay
with their owners; task/flow lifecycle bindings and inspection joins retire.
See [state schema 19](/reference/database-schemas/state-schema-history#state-schema-19)
for compatibility, rollback, and migration limits. Cron receipts remain the
execution and recovery authority; history never authorizes launch or adoption.
