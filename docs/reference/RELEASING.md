...
Flaky tests never block npm/ClawHub publication through ordinary CI, plugin, cross-OS, performance, or QA evidence: those suites are advisory without a lane waiver. Record the first failing attempt as evidence, diagnose the owning defect, and fix it at its owner on `main`; automatic retries were removed, and a passing replay alone does not prove a fix.
