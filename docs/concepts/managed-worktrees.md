...
An orphan retired without a snapshot no longer appears in `worktrees list` and cannot use `worktrees restore`. GC reports the preserved checkout path for manual recovery. Keep those files and the retained source branch; recover work into a separate checkout instead of deleting or overwriting the preserved files. Retirement does not reconstruct missing Git metadata or create a replacement snapshot.
