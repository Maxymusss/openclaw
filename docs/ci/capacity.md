...
PR run `35826932121` exposed a 297-second plugin estimate whose eight serial children actually took 1,225 seconds. Complete successful process spans from that run refresh the Teams, media, and IRC rates and set floors for exact costly selections in split configs. Each measured floor includes 10% pricing headroom plus two seconds of process admission; the higher canonical estimate still wins. Selection floors bind the config and complete ordered file list, so a different inventory expires the observation instead of charging every sibling for one expensive selection. The 500-second aggregate work budget uses these corrected prices while preserving the separate 20-file database-worker ceiling and preparation boundaries.
...
Eligible Blacksmith and hybrid compact bins with multiple ordinary groups request the existing 32-vCPU runner with two child-process slots. Initial packing retains the 360-second aggregate budget and completes Gateway serialization, runtime placement, and worker pinning before compaction. The original 500-second aggregate pass settles placement and worker policy first. After command repricing, jobs already admitted on the 32-class with two process slots are repacked within compatible environments to 600-second elapsed budgets on PRs and 720 seconds on main. Replacements keep the shortest existing job deadline, including the implicit 60-minute default; no child gains time.

Admission simulates the executor’s ordered next-free-slot scheduling; dividing summed work by two can miss the final uneven child. Exact native two-slot prices stay separate from serial observations, and unmatched children use the canonical estimate with 25% headroom and two seconds of admission. Those jobs may share capacity across logical runner classes while each child retains its original runner metadata and worker ceiling. Serial placement retains each original ordered process envelope; widening the initial packing can concentrate slow Gateway work even when its final estimate remains below 360 seconds.

A final measured pass can repack intact serial child processes on the same runner, with compatible job environment and without runtime preparation. Serial replacements also retain the shortest existing deadline. Exact runner, worker override, and child descriptors bind the observations; changed selectors or execution policy expire them. Unmatched 32-class children use the canonical estimate with 25% pricing headroom and two seconds of process admission. Other classes still require complete observations. A row participates only when every child has an admitted price. Node main and Bun-compatible PR observations stay separate. Both final passes reserve 110 seconds once per job for setup and wrapper overhead inside the 720-second main or 600-second PR target. Complete known prices remain visible even when a row cannot be compacted; equal-count redistribution may repair an over-budget row. A row-level two-worker ceiling moves onto each child before sharing a default-capacity row; lower child limits remain lower. Other environment overrides stay in separate cohorts. Every child retains its config, include set, effective environment, memory fallback, worker ceiling, and process boundary.

Compatible parallel groups can fill the new budget without the ten-group cutoff retained by initial serial packing. Final measured serial packing uses complete child wall prices and the elapsed budget. Blacksmith serial jobs retain their 200/276-second budgets; hybrid serial jobs retain 210 seconds. Exclusive jobs retain 150 seconds by default. Only complete ordinary hybrid bins of non-build CLI groups may use 250 seconds and share split siblings; every child must still fit 150 seconds. Groups above their existing serial cap stay alone. Exclusive groups, original single groups, dist descriptors and jobs with runtime preparation remain serial. Hybrid exclusive and dist bins retain their existing prerequisite sharing. The shard executor admits at most two processes only when the actual host has at least eight available CPUs and 24 GiB of memory; smaller capacity admits one. Each overlapping child keeps two Vitest workers, including a singleton left after compaction; inner project parallelism remains one. Commands files follow the measured worker allocation in serial self-hosted rows, with a two-worker fallback for hosted, constrained, frozen, or overlapping execution; the existing non-isolated fork runner retains file-boundary cleanup. The primary `github` profile stays serial at 210 seconds. Preflight records the actual row count for each source revision; canonical inventory comparisons must preserve every original child plan and test input. Native elapsed-time, memory and cleanup evidence must establish the actual effect.
...
The September 23 baseline jobs spent a median 31.5–49 seconds outside the Node test step: checkout medians were 9–26 seconds and Node setup was 14 seconds. One checkout-heavy row spent 155 seconds outside tests, so the initial 60-second reserve was an admission estimate, not a deadline guarantee. Main observations use each child’s largest complete successful span, rounded up with two seconds for process admission, across runs `35810905247`, `35811288334`, and `35811746607`; PR observations come from Bun-compatible run `35811104672`. Count every REST jobs page when measuring savings: the main baseline had 40 Blacksmith 32-class jobs, including one beyond the first 100 job records. The eight UI rows took 385–447 seconds in run `35811567317`. A later seven-row trial in `35844334796` exceeded 600 seconds in six rows and 720 seconds in two, including a successful 771-second row. Ordinary CI therefore retains eight Control UI rows with three parallel workers and one serial worker. The release-tier inventory change removed no Control UI files; the smaller Node inventory does not justify shrinking these browser rows. Exact-head Actions runs must establish the resulting job and workflow wall times.

Later exact-head qualification runs `35826222393`, `35826932121`, and `35844334796` refresh only unchanged execution contracts. The last run measured up to 103 seconds outside tests plus six seconds of wrapper work, motivating the 110-second reserve. Those refreshed prices retain the larger successful child span, add 10% pricing headroom, round up, and include two seconds of process admission. Repricing a command worker allocation carries its adjusted cost with the replacement group so later serial packing cannot read a missing timing key as zero. Inventory changes invalidate old selector contracts and retain the canonical fallback estimate. The 390-file CLI subset was separately measured on the matching eight-CPU, 33-GB allocation in Testbox run `35855504751`: 271.36 seconds for main policy and 278.76 for PR policy, priced at 301 and 309 seconds after the same margin. The native variation includes larger outliers than 10%, so this allowance is a packing estimate; the exact candidate still needs native row and workflow timing proof.

Expanded serial large/small jobs admit 210 predicted seconds; eligible hybrid parallel bins initially admit 360 before final parallel compaction to 600 elapsed seconds on PRs or 720 on main, including the shared 110-second overhead reserve. All profiles retain the shared 90-row compact cap. The 150-second file-split and default exclusive-group budgets stay unchanged; complete non-build CLI bins alone may use the 250-second ordinary hybrid admission budget. The PR-only performance lifecycle file retains its 136-second fallback from native spans of 127.288/135.808 seconds in runs 33532741896/33545657559; canonical pushes omit that tooling family. Trusted contributor forks can use the GitHub profile on Blacksmith, so every profile participates in the same registration bound. The widest current workflow profiles retain up to 87 other potential rows (14 nonmatrix and 73 matrix), or 88 for historical targets without the UI named-project contract. The conservative cap-based envelope already includes twelve Control UI shards plus the browser-extension row on every profile. Excluding the four unconditionally hosted iOS rows, three hosted macOS Swift phases, and the hosted aggregate gate gives the conservative ceiling of 80 potentially eligible rows. This includes the new Control UI performance job; keep the ceiling rather than spending savings from consolidated checks. With the final Node caps, the bounds are 150 registrations per main run and 210 per PR. Two active main slots, both pending successors and the observed peak of 21 non-skipped PR arrivals give `4 × 150 + 21 × 210 = 5,010` registrations in five minutes. This leaves 990 within the 6,000 reference operating target for release work, adjacent repositories and carryover; it does not prove those arrivals fit. The earlier 19-arrival estimate is obsolete. Using the prior 4,826-registration reference, the bounded 2026-09-02 cohort audit counted 321 unassigned Blacksmith jobs and reserved nine auxiliary rows, giving `4,826 + 321 + 9 = 5,156` planned registrations and an 844-row allowance below that reference. Its 40 exact attempts covered 4,830 jobs; queued observations spanned 21:50:48–21:57:11 UTC and were not simultaneous. Already-assigned jobs, old approval-waiting runs, unobserved retries and unlisted organization work remain outside that cohort, so this is a conditional planning bound rather than a live organization balance. Evaluate a single PR trial using its actual emitted rows separately from the rollout model. Budget all six npm qualification jobs and the relevant full-release children; a shared-token quota response or unused bucket does not establish organization-wide usage or physical runner capacity.
