...
Measured serial Gateway-core rows can use the [16-class with unchanged two-worker ceilings](/ci/runners#blacksmith-runner-capacity); envelopes containing UI tests, memory-gated, overlapping, build, and fuller packed rows retain their existing capacity.
