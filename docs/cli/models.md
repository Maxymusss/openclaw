...
Each model entry can have an `alias` string and an `aliases` array. Existing single-alias configurations work unchanged. `add` keeps the existing name and appends additional names; adding the same name with different capitalization updates its spelling. `remove` removes only the named alias, case-insensitively, without removing the model or its other settings.
...
```json5
{
  agents: {
    defaults: {
      models: {
        "anthropic/claude-sonnet-4-6": { alias: "chat", aliases: ["writing", "fast"] },
      },
    },
  },
}
...
All three names resolve to the same model, including `/model writing` and `/writing` in chat. Built-in commands and skill names take precedence over alias shorthand. `add` resolves `<model-or-alias>` to a canonical provider/model key first, so aliases do not form chains.
Per-agent model entries support the same fields; an explicit per-agent alias list replaces the inherited names for that model. Older OpenClaw versions that do not support `aliases` require removing that field before downgrading.
