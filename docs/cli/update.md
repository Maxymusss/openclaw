...
Before state schema 19 is applied, update admission recognizes orphan
`task_delivery_state` rows whose parent tasks are missing as repairable. When it
can acquire Doctor's ownership fences, it runs
...
structural damage still refuse admission. This existing-schema admission does not
advance schema markers or drop Task tables. The later fenced
[state schema 19 migration](/reference/database-schemas/state-schema-history#state-schema-19)
retires those tables after preserving cron history and exact native outcomes.
