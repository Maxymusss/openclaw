...
- `gateway.roles.definitions.<role>.models.allow`: optional array of exact `provider/model` references that adds a model ceiling. Omission preserves existing behavior; `[]` denies all model inference. Wildcard model references are not accepted, while `agents: "*"` remains valid. This ceiling does not grant scopes, accounts, or runtime access. The built-in OpenClaw runtime enforces exact model ceilings; the selected physical provider route and transport must also support enforcement. Unsupported routes are refused without substitution. See [Operator model ceilings](/gateway/operator-scopes#optional-model-ceiling).
- When both model ceilings are configured, a model must satisfy both. A live model-policy edit cannot widen work already admitted under its original policy.
