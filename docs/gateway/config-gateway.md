...
- `gateway.roles.definitions.<role>.models.allow`: optional array of exact `provider/model` references that adds a model ceiling. Omission preserves existing behavior; `[]` denies all model inference. Wildcard model references are not accepted, while `agents: "*"` remains valid. This ceiling does not grant scopes, accounts, or runtime access. Finite runtime support remains unqualified and the built-in runtime does not declare it; do not enable this field for deployed visitors until the [model ceiling release requirements](/gateway/operator-scopes#optional-model-ceiling) are met.
