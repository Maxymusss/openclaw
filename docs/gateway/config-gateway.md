...
- Configured `modelPolicy` also requires the selected runtime, physical provider route, and transport to enforce the retained request. The host binds the authorized logical selection to its prepared physical route and provider-owned wire model. Unsupported routes are refused without substitution, including explicit `auto` and unqualified WebSocket routes. This tightens enforcement for existing configured model policies; qualify the deployed route before enabling it. Omission retains ordinary unrestricted behavior. See [Operator model ceilings](/gateway/operator-scopes#optional-model-ceiling).
