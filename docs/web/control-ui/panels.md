...
<a id="github-side-panel" />

## GitHub links

GitHub issue, pull-request, and commit links use the same [Browser panel](/web/control-ui/panels#browser-panel)
route as other external links. The panel displays the actual GitHub website,
not an API-rendered reader. In the macOS app, unmodified links open native Mac
tabs. In a regular browser, enable **Open links in Control UI browser** under
**Settings → Infrastructure → Browser** to open Agent browser tabs; otherwise
links keep normal browser navigation.

The bundled GitHub preview/reader plugin has been removed. GitHub publishing,
managed identities, and session pull-request status remain available. See the
[GitHub upgrade note](/plugins/github) for existing plugin configuration.
