...
import { readCronRunHistoryPage } from "../../../cron/run-history.test-support.js";
...
it("archives JSONL only after native history commits and imports it once", async () => {
  await withOpenClawTestState(
    ...
    async (state) => {
      ...
      database.db.exec(
        "CREATE TRIGGER reject_history BEFORE INSERT ON cron_run_history BEGIN SELECT RAISE(ABORT, 'history-write-refused'); END;",
      );
      ...
      expect(
        readCronRunHistoryPage({ storeKey: cronStoreKey(storePath), jobId: "legacy" }).entries,
      ).toMatchObject([entry]);
      await expect(migrateLegacyCronRunLogsToSqlite(storePath)).resolves.toEqual({
        importedFiles: 0,
      });
      expect(
        readCronRunHistoryPage({ storeKey: cronStoreKey(storePath), jobId: "legacy" }).total,
      ).toBe(1);
    },
  );
});
