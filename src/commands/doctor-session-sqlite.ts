...
import { archiveLegacyArtifacts } from "./doctor-session-sqlite-archive-publication.js";
...
import {
  ...
  type CompletedLegacyStoreReplay,
...
export async function runDoctorSessionSqlite(
  ...
  const settlements =
    options.mode === "import" || options.mode === "recover"
      ...
      : [];
  const historicalEvidence = ["import", "dry-run", "validate", "recover"].includes(options.mode)
  ...
  if (activeRun && coverage) {
    ...
    // Replay refusals keep their original names, including paths also reported as
    // plugin conflicts. Protect them before that separate owner may archive inputs.
    for (const owner of archiveTargets) {
      for (const source of owner.replayRetainedPaths ?? []) {
        const canonical = canonicalMigrationFilePath(source);
        coverage.retainedPaths.add(canonical);
        deferredSourcePaths.add(canonical);
        if (canonical === owner.target.storePath) {
          coverage.selectedStorePaths.delete(canonical);
        }
      }
    }
    for (const owner of archiveTargets) {
...
async function inspectOrMigrateTarget(params: {
  ...
  completedStoreReplays?: readonly CompletedLegacyStoreReplay[];
  ...
  const replayRetainedPaths = new Set<string>();
  ...
  const matchesTarget = (candidate: CompletedLegacyStoreReplay["target"]) =>
    ...
    params.archiveTargets?.push({
      ...
      records: records
        .filter(
          (record) =>
            !record.historical?.archiveMove &&
            !sourceConflicts.has(record.transcriptPath ?? "") &&
            !replayRetainedPaths.has(record.transcriptPath ?? ""),
        )
...
function validateLegacySessionRecord(
  ...
  if (
    record.completedIndexReplay?.outcome &&
    record.completedIndexReplay.outcome !== "appended" &&
    record.completedIndexReplay.outcome !== "unchanged"
  ) {
    // Intentional replay refusal preserves a newer, deleted, or otherwise unproven owner.
    return;
  }
