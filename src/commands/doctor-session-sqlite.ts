  ...
  if (activeRun && coverage) {
    ...
    for (const owner of archiveTargets) {
      for (const source of owner.sourceConflicts ?? []) {
        const canonicalSource = canonicalMigrationFilePath(source);
        coverage.retainedPaths.add(canonicalSource);
        deferredSourcePaths.add(canonicalSource);
        if (canonicalSource === owner.target.storePath) {
...
async function inspectOrMigrateTarget(params: {
  ...
  if (retainedImport) {
    ...
    await importLegacySessionRecords(params.target, records, report, sourceConflicts);
