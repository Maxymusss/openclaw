...
export function resolveFollowupDeliveryContextKey(run: FollowupRun): string {
  ...
  return JSON.stringify([
    ...
    stableStringify(execution.deferredReplyModelLevels?.selection ?? null),
    execution.deferredReplyModelLevels?.explicitThink ?? false,
    execution.deferredReplyModelLevels?.thinking.configuredThinkingDefault ?? null,
    execution.fastMode ?? "",
