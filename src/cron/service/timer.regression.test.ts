...
describe("cron service timer regressions", () => {
    ...
    async (timeoutSeconds) => {
      ...
      try {
        ...
        const taskRunId = receipt
          ? `cron:${cronJob.id}:${receipt.startedAtMs}:${receipt.receiptId}`
          : undefined;
  ...
  it("keeps timed-out cron runs from being overwritten by late cancellation", async () => {
    ...
    try {
      ...
      expect(cancelActiveCronTaskRun({ runId, reason: "Cancelled by operator." })).toBe(false);
  ...
  it.each([false, true])(
    ...
    async (withCondition) => {
      ...
      try {
        ...
        const admittedRunId = requireAdmittedRunId(store.storePath, cronJob.id);
        expect(
          cancelActiveCronTaskRun({ runId: admittedRunId, reason: "Cancelled by operator." }),
        ).toBe(false);
        expect(
