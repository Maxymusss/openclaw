            ...
            return withOpenClawStateReadOnlyLocation(
              ({ db }, schemaFacts) => {
                ...
                if (command.type === "audit.run.inspect") {
                  const schema = schemaFacts.executionIdentityInspection;
                  if (!schema) {
                    throw new Error("Audit inspection requires admitted schema facts");
                  }
                  try {
                    return {
                      ok: true,
                      ...
                      result: {
                        status: "inspected",
                        inspection: inspectExecutionIdentityRunInDatabase(
                          db,
                          command.input,
                          schema,
                        ),
              ...
              input.snapshotRoot,
              true,
              command.type === "audit.run.inspect" ? "executionIdentityInspection" : undefined,
            );
          },
        ),
      );
