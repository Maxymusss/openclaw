...
vi.mock("./registry.js", async (importActual) => {
  const actual = await importActual<typeof import("./registry.js")>();
  return {
    ...actual,
    assertSandboxRuntimeRetirementAllowed: vi.fn(
      (entry: Pick<SandboxRegistryEntry, "containerName" | "retirementPolicy">) => {
        expect(entry.retirementPolicy).toBeUndefined();
        expect(fixture.registry.get(entry.containerName)?.retirementPolicy).toBeUndefined();
      },
    ),
    ...
    completeSandboxRegistryReservation: vi.fn(
      async (expected: SandboxRegistryEntry, entry?: SandboxRegistryEntry) => {
        expect(fixture.registry.get(expected.containerName)).toEqual(expected);
        if (!entry) {
          fixture.registry.delete(expected.containerName);
          return;
        }
        fixture.registry.set(entry.containerName, {
          ...fixture.registry.get(entry.containerName),
...
vi.mock("./container-engine.js", async (importActual) => {
  const actual = await importActual<typeof import("./container-engine.js")>();
  return {
    ...actual,
    ...
    execContainer: vi.fn(
      async (_engine: unknown, args: string[], opts?: ExecContainerRawOptions) => {
