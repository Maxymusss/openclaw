...
export async function runExecProcess({
  ...
  const prepareSpawnSpec = async () => {
    if (opts.sandbox) {
      ...
      const cleanup = ownsLocalCli
        ? undefined
        : opts.sandbox.prepareProcessCleanup?.(shellRuntimeEnv);
  ...
  try {
    ...
    // Capture the private generation before preparation; it alone owns native
    // cleanup, while ordinary sandboxes keep their process-token cleanup owner.
    ownsLocalCli =
