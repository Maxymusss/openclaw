...
describe("persisted subagent requester wakes", () => {
      ...
      if (cut === "second owner") {
        ...
        database.db.exec(
          cut === "second run write"
            ? "CREATE TEMP TRIGGER reject_batch AFTER UPDATE ON subagent_runs WHEN NEW.run_id = 'completion-second' BEGIN SELECT RAISE(ABORT, 'cut:second'); END"
  ...
  it.each(["current", "replacement", "rearm"] as const)(
    ...
    async (owner) => {
      ...
      database.db.exec(
        "CREATE TEMP TRIGGER reject_delivered AFTER UPDATE ON subagent_runs BEGIN SELECT RAISE(ABORT, 'cut:delivered'); END",
      );
      driver.wake.mockImplementation(async (params) => {
  ...
  it.each([
    ...
    async ({ change, delivered }) => {
      ...
      database.db.exec(
        "CREATE TEMP TRIGGER reject_outcome AFTER UPDATE ON subagent_runs BEGIN SELECT RAISE(ABORT, 'cut:outcome'); END",
      );
      try {
        await driver.run();
        expect(driver.wake).toHaveBeenCalledOnce();
  ...
  it.each(["not_required", "discarded"] as const)(
    ...
    (status) => {
      ...
      database.db.exec(
        "CREATE TEMP TRIGGER reject_closed_wake AFTER UPDATE ON subagent_runs BEGIN SELECT RAISE(ABORT, 'temporary closed-wake write failure'); END",
      );
      try {
        for (let attempt = 0; attempt < 6; attempt += 1) {
          expect(settle).toThrow("temporary closed-wake write failure");
