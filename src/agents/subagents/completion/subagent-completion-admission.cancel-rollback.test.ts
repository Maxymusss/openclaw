  ...
  it.each([
    ...
    async ({ outcome, owner }) => {
      ...
      if (outcome === "delivered") {
        database.db.exec(
          "CREATE TEMP TRIGGER reject_delivered AFTER UPDATE ON subagent_runs WHEN json_extract(NEW.payload_json, '$.delivery.status') = 'delivered' BEGIN SELECT RAISE(ABORT, 'cut:delivered'); END",
        );
      }
