...
describe("requester receipts after completion expiry", () => {
  ...
  it.each(["run", "execution"] as const)("rejects a changed native %s owner", (changedOwner) => {
    const input = suspend();
    const changed = structuredClone(input.subagent);
    if (changedOwner === "run") {
  ...
  it("rolls back delivery and wake changes together if the registry write fails", () => {
    const input = suspend();
    database.db.exec(
