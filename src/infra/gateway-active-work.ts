...
export function createGatewayActiveWorkSnapshot(
  ...
  add(counts.agentRuns, "agent-run", `${counts.agentRuns} admitted agent run(s)`);
  add(counts.acpRuns, "acp-run", `${counts.acpRuns} active ACP turn(s)`);
  add(counts.mediaRuns, "media-generation", `${counts.mediaRuns} active media generation(s)`);
