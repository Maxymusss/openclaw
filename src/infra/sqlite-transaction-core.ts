// The transaction engine owns SQLite state; callers own warning delivery.
// Workers with explicit reporters must not load the default logging runtime.
...
type SqliteTransactionContext = SqliteTransactionOptions & {
...
function commitImmediateTransaction(db: DatabaseSync, options: SqliteTransactionContext): void {
  execTimedTransactionStep({
    db,
    options,
    sql: "COMMIT",
