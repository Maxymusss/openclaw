...
describe("listGatewayMethods", () => {
  ...
  it("appends new methods after model probing while preserving retained older method order", () => {
    ...
    const retainedLegacyMethods = LEGACY_ADVERTISED_GATEWAY_METHODS.filter(
      (method) => !method.startsWith("tasks."),
    );
    const legacyCount = retainedLegacyMethods.length;
    expect(methods.some((method) => method.startsWith("tasks."))).toBe(false);

    expect(methods.slice(0, legacyCount)).toEqual(retainedLegacyMethods);
