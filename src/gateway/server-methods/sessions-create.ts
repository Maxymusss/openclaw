...
export const sessionCreateHandlers: GatewayRequestHandlers = {
  "sessions.create": handleSessionCreate,
};

async function handleSessionCreate(options: GatewayRequestHandlerOptions): Promise<void> {
  const {
    params,
    respond,
    context,
    ...
    const created = await createGatewaySession({
      ...
      afterCreate: async (session) => {
        ...
        const sendOptions = bindGatewayRequestHandlerMutationAuthority(
          ...
          {
            ...options,
            // A model-policy-only reload may widen a fresh capture without revoking
            // this request. Only its nested send inherits the original selection ceiling.
            client:
              operatorAuthority && client
                ? {
                    ...client,
                    internal: { ...client.internal, operatorRunAuthority: operatorAuthority },
                  }
                : client,
            params: {
