...
describe("update.run unexpected-error diagnostics", () => {
  it.each(["restored", "unconfirmed", "rejected"] as const)(
    "joins cancellation and preserves projection failure when cancellation is %s",
    async (outcome) => {
      const original = new Error("result projection failed");
      const cancellationEntered = createDeferred();
      const cancellationRelease = createDeferred();
      ...
      cancelManagedServiceUpdateHandoffMock.mockImplementationOnce(async () => {
        ...
        return outcome === "restored" ? "restored-in-process" : false;
      });
      const respond = vi.fn();
      const logGateway = {
        info: vi.fn(),
        warn: vi.fn(() => {
          throw new Error("secondary warning failure");
        }),
      };
      let settled = false;
      const pending = invokeUpdateRun({}, respond, undefined, { logGateway }).then(
        () => {
          settled = true;
          return undefined;
        },
      ...
      try {
        ...
        expect(respond).not.toHaveBeenCalled();
        expect(logGateway.warn).toHaveBeenCalledTimes(outcome === "restored" ? 0 : 1);
      } finally {
        cancellationRelease.resolve();
        await pending;
