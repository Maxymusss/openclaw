...
async function withHeldState(
    ...
    .track(
      withOpenClawTestState({ scenario: "minimal" }, async (state) => {
        ...
        try {
          ...
          await body(state, track);
        } catch (error) {
          failures.push(error);
        }
        // State teardown must follow the actual producers, including assertion failures.
        try {
          release();
  ...
  if (!entered && failures.length === 0) failures.push(new Error("state fixture did not enter"));
  if (failures.length === 1) throw failures[0];
  if (failures.length > 1)
    throw new AggregateError(failures, "state fixture failed", { cause: failures[0] });
}
        ...
        const fetchImpl = routedFetch([
          {
            ...
            response: () =>
              failed
                ...
                : githubJson([
                    pullListItem({ state: "closed", merged_at: "2026-09-01T00:00:00Z" }),
                  ]),
          },
        ]);
        const load = () =>
      ...
      async (fixtureOwner) => {
        ...
        await withHeldState(
          ...
          async (state, track) => {
            ...
            releaseHeld = release.resolve;
            fixtureOwner.signal.throwIfAborted();
            ...
            try {
              ...
              const replacement = await track(loadReplacement());
              expect.soft(replacement.pullRequests[0]?.title).toBe("Replacement session result");
              expect(fetchImpl.mock.calls).toHaveLength(1);
              expect(await track(loadReplacement())).toEqual(replacement);
              expect(fetchImpl.mock.calls).toHaveLength(1);
            } finally {
              release.resolve();
              await pending;
        ...
        await withHeldState(
          ...
          async (state, track) => {
            ...
            try {
              ...
              owner.signal.throwIfAborted();
              expect(await track(load())).toEqual(result);
              expect(fetchImpl).toHaveBeenCalledTimes(1);
            } finally {
              release.resolve();
              await pending.catch(() => {});
            ...
            async (state, track) => {
              ...
              for (const storePath of [original, replacement]) {
                await upsertSessionEntryCore(
                  ...
                  {
                    sessionId: "identical-pr-session",
                    updatedAt: 1,
                    spawnedCwd: state.workspaceDir,
                  },
                );
