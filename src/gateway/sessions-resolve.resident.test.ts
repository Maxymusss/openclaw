    ...
    await withOpenClawTestState({ scenario: "minimal" }, async (state) => {
      ...
      try {
        ...
        for (const change of [{ archivedAt: 2 }, { visibility: "draft" as const }]) {
          ...
          await projection.prepareExactRows([{ agentId: "main", key: sentinel }]);
          expect(projection.describe({ agentId: "main", key: sentinel })?.entry.sessionId).toBe(
            winner.sessionId,
          );
        }
...
it("prepares a cold exact resolution and refuses consumption after its projection owner changes", async () => {
  await withOpenClawTestState({ scenario: "minimal" }, async () => {
    const key = "agent:main:archived-resolve";
    replaceSessionEntrySync(
    ...
    try {
      expect(projection.describe({ agentId: "main", key })).toBeUndefined();
      expect(
        await withPreparedSessionResolve({ projection, client: null, p: { key } }, consume),
      ).toMatchObject({ ok: true, key, agentId: "main" });
      consume.mockClear();
      await expect(
        withPreparedSessionResolve(
          {
