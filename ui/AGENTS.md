...
- Fixed-size single-SVG Grid controls must set explicit padding and leave enough content space for the icon. `openclaw/icon-grid-fit` checks plain CSS through the existing Stylelint lane; `pnpm lint:ui:icon-grid` runs the same scanner as a standalone JSON gate. For an advisory scan, use `node scripts/audit-control-ui-icon-buttons.mts [--root <checkout>] [<css-file> ...]`. The scan joins literal Lit template ancestry with baseline stylesheet rules and the shared base/component styles. Its narrow scope is pixel-sized border-box controls with implicit Grid tracks and centered items. Dynamic selectors/classes, non-pixel or logical geometry, `!important`, inline styles, conditional/nested rules, cross-sheet overrides, explicit tracks, shadow/custom roots, and Flex layouts require browser proof; skipped cases are not proof of correct layout. Do not use Stylelint `--cache` for this rule: findings also depend on current templates and shared stylesheets. Validate new detection shapes in a real browser instead of broadening guesses or adding suppressions.
