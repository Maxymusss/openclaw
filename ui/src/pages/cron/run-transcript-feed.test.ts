...
import { renderCronTranscriptFeed } from "./run-transcript-feed.ts";

afterEach(() => document.body.replaceChildren());

function mount(messages: unknown[]) {
  const container = document.body.appendChild(document.createElement("div"));
  render(renderCronTranscriptFeed(messages), container);
  return container;
}
...
describe("cron transcript feed", () => {
  ...
  it("groups consecutive calls across result messages and expands every command or path", () => {
    ...
    const groups = container.querySelectorAll<HTMLDetailsElement>(
      "details.cron-transcript-feed__tool-group",
    );
    ...
    expect(
      group.querySelector(".cron-transcript-feed__calls .cron-transcript-feed__tool-line--full")
        ?.textContent,
    ...
    expect(
      group.querySelectorAll(".cron-transcript-feed__tool-line").length,
  ...
  it.each(["arguments", "args", "input"])(
    ...
    (argumentField) => {
      ...
      const groups = container.querySelectorAll(".cron-transcript-feed__tool-group");
      expect(groups).toHaveLength(1);
      expect(groups[0]?.querySelector("summary")?.textContent?.trim()).toBe("Raw details");
      expect(
        [...groups[0]!.querySelectorAll(".cron-transcript-feed__tool-line--full")].map((line) =>
  ...
  it("preserves mixed-block order and associates late failure results with their call", () => {
    ...
    const entries = [...container.querySelectorAll(".cron-transcript-feed__entry")];
    ...
    const failed = [...container.querySelectorAll(".cron-transcript-feed__tool-line")].filter(
      (line) => line.textContent?.includes("pnpm check:ui"),
    );
    expect(failed.length).toBeGreaterThan(0);
    expect(failed.every((line) => line.classList.contains("cron-transcript-feed__error"))).toBe(
      true,
    );
    const successful = [...container.querySelectorAll(".cron-transcript-feed__tool-line")].find(
      (line) => line.textContent?.includes("ui/package.json"),
    );
    expect(successful?.classList.contains("cron-transcript-feed__error")).toBe(false);
  });
  ...
  it.each(["tool", "function", "toolResult", "tool_result", undefined])(
    ...
    (role) => {
      ...
      expect(container.querySelectorAll(".cron-transcript-feed__entry")).toHaveLength(1);
      expect(container.querySelector(".cron-transcript-feed__tool-group")).toBeNull();
    },
  );
  ...
  it("omits commentary text while retaining a call in the same assistant message", () => {
    ...
    expect(container.textContent).not.toContain("Result body must not appear");
    expect(container.querySelectorAll(".cron-transcript-feed__tool-group")).toHaveLength(1);
    expect(
      container.querySelector(".cron-transcript-feed__tool-group summary")?.textContent,
    ).toContain("Raw details");
    expect(container.textContent).toContain("The stylesheet owns the layout.");
    expect(container.querySelectorAll(".cron-transcript-feed__entry")).toHaveLength(2);
  });
  ...
  it("shows local HH:MM message timestamps and omits missing timestamps", () => {
    ...
    expect(
      [...container.querySelectorAll(".cron-transcript-feed__time")].map(
        (time) => time.textContent,
      ),
    ).toEqual(["09:07", "09:08"]);
    const unclocked = [...container.querySelectorAll(".cron-transcript-feed__entry")].find(
      (entry) => entry.textContent?.includes("No timestamp"),
    );
    expect(unclocked?.querySelector(".cron-transcript-feed__time")).toBeNull();
  });
  ...
  it.each(["set -e", "export FOO=bar", "unset FOO"])(
    ...
    (command) => {
      ...
      expect(container.querySelector(".cron-transcript-feed__row-label")?.textContent).toBe(
        command,
      );
    },
  );
  ...
  it("redacts a complete credential-shaped fixture before shortening the command label", () => {
    ...
    expect(container.querySelector(".cron-transcript-feed__row-label")?.textContent).not.toContain(
  ...
  it("preserves anonymous command disclosures when earlier rows arrive in the same group", () => {
    ...
    const original = container.querySelectorAll<HTMLDetailsElement>(
      ".cron-transcript-feed__tool-line",
    );
    original[2]!.querySelector("summary")!.click();
    expect(original[2]!.open).toBe(true);
    render(
      renderCronTranscriptFeed([first, anonymous("inserted-message", "echo inserted"), one, two]),
      container,
    );
    const current = container.querySelectorAll<HTMLDetailsElement>(
      ".cron-transcript-feed__tool-line",
    );
