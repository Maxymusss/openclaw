...
function renderToolLine(call: ToolCard) {
  ...
  return html`<details
    class="cron-transcript-feed__tool-line ${isToolCardError(call) ? "cron-transcript-feed__error" : ""}"
  >
    <summary>
      <span class="cron-transcript-feed__row-icon" aria-hidden="true">${toolIcon(call)}</span>
      <span class="cron-transcript-feed__row-label">${label}</span>
      <span class="cron-transcript-feed__row-outcome" title=${outcomeLabel}
        >${outcome === "succeeded" ? html`<span role="img" aria-label=${outcomeLabel}>${icons.check}</span>` : outcomeLabel}</span
      >
      <span class="cron-transcript-feed__chevron" aria-hidden="true">${icons.chevronRight}</span>
    </summary>
    <pre class="cron-transcript-feed__tool-line--full"><code>${raw}</code></pre>
  </details>`;
}

function renderToolGroup(entry: Extract<Entry, { kind: "tools" }>) {
  const overview = describeToolGroup(entry.activity);
  return html`<details class="cron-transcript-feed__tool-group">
    <summary>
      <span class="cron-transcript-feed__overview">
        <span class="cron-transcript-feed__overview-heading">
          <strong
          ...
          >
          ${overview.outcomes.map(({ kind, label }) => html`<span class="cron-transcript-feed__outcome cron-transcript-feed__outcome--${kind}">${label} </span>`)}
        </span>
        ${overview.label ? html`<span class="cron-transcript-feed__summary">${overview.label}</span>` : nothing}
      </span>
      <span class="cron-transcript-feed__chevron" aria-hidden="true">${icons.chevronRight}</span>
    </summary>
    <div class="cron-transcript-feed__calls">
      ${repeat(
        entry.calls,
        ({ key }) => key,
...
export function renderCronTranscriptFeed(messages: unknown[]): TemplateResult {
  return html`<div class="cron-transcript-feed">
    ${repeat(
      entries(messages),
      (entry) => entry.key,
      (entry) => html` <div
        class="cron-transcript-feed__entry"
        data-cron-transcript-entry=${entry.key}
      >
        <span class="cron-transcript-feed__icon" aria-hidden="true"
          ...
          ${
            entry.kind === "tools"
              ...
              : entry.kind === "assistant"
                ...
                : html`<div class="cron-transcript-feed__${entry.kind}">${entry.text}</div>`
          }
        </div>
        ${entry.timestamp !== null ? html`<time class="cron-transcript-feed__time" datetime=${new Date(entry.timestamp).toISOString()}>${new Date(entry.timestamp).toLocaleTimeString([], { hour: "2-digit", minute: "2-digit", hour12: false })}</time>` : nothing}
      </div>`,
    )}
  </div>`;
}
