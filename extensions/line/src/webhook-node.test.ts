  ...
  await params.handler(
    {
      ...
      // Middleware-parsed fields must never replace the signed raw request.
      body: { events: [{ type: "message", source: { userId: "tampered-user" } }] },
...
async function invokeWebhookPost(params: { failWith?: unknown; rawBody: string; signed: boolean }) {
...
describe("LINE webhook POST contract", () => {
  it("rejects verification-shaped requests without a signature", async () => {
    const result = await invokeWebhookPost({
...
describe("createLineNodeWebhookHandler", () => {
  ...
  it("dispatches the signed raw body rather than a middleware-parsed body", async () => {
    const event = { type: "message", source: { userId: "signed-user" } };
