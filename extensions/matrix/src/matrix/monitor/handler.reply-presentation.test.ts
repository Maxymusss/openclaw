...
describe("matrix monitor handler reply presentation", () => {
  ...
  it.each([undefined, "off"] as const)(
    ...
    matrixFixture.wrapCase(async (accountScope) => {
  ...
  it(
    "resolves a reply's presentation before the room delivery reads it",
  ...
  it(
    "edits a matching live preview to attach the final reply's controls",
    matrixFixture.wrapCase(async () => {
      const context = {
        cfg: { channels: { matrix: {} } },
