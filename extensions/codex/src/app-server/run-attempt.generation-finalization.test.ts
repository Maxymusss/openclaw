...
describe("Codex finalization generation ownership", () => {
  it.each(["current", "lineage-replaced"] as const)(
    ...
    async (generation) => {
      ...
      let guarded = false;
      harness.request.mockImplementation((method, request, options) => {
        if (method !== "turn/start") return originalRequest(method, request, options);
        expect(options?.withCurrent).toBeTypeOf("function");
        const withCurrent = options!.withCurrent!;
        return originalRequest(method, request, {
          ...options,
