...
- **Prompt-cache preservation.** Changes to prompt/bootstrap assembly, tool definitions, transcript replay/projection, provider serialization, or warm-session identity must assess cache impact. Preserve unchanged cacheable prefixes and previously sent history; document intentional invalidation permitted by existing lifecycle contracts and its tradeoff in the PR's Evidence section. Reuse existing coverage; extend it when an affected transition (such as tool replay, resume, heartbeat, or metadata changes) is not covered. Verify consecutive turns at the final provider-request or session-reuse boundary; cache-regression tests must fail before the fix. For material cache-affecting SDK, transport, or cache-policy changes, include scoped live before/after cache-read/write counts for individual warm turns, not just aggregate hit rates. Use provider-specific expectations, not a universal hit-rate threshold. Unrelated changes need no cache benchmark.
